...
- **Phase:** Sprint 10 complete and merged — Hugging Face only: Deterministic, Online AI and Local AI modes with a corroborated token (`OAK-S10-001`–`008`), authorized by the owner on 2026-09-21, merged as PR #22 (`052211e`). Sprint 9 (`OAK-S9-001`–`009`) is merged as PR #21 (`88fa876`). `0.8.0` is **tagged `v0.8.0`, approvals unsigned**: the touch-list, changelog section, decision record and evidence are in the tree, the tag was cut on 2026-09-22 on the owner's instruction, and the named maintainer, security and licence rows in `docs/release/0.8.0/release-decision.md` are still empty. Nothing is published. `0.7.1` was approved by `nmasamba` on 2026-08-27 and **published as a GitHub Release on 2026-09-03**; nothing about it changes here
...
| `OAK-S9-005` | complete | Standard-library transport with a fixed egress allowlist, seven provider profiles (narrowed to two in Sprint 10), hosted interpreter, Hugging Face free-model lookup, egress gates rewritten, `TM-13` direct, `RR-039`/`RR-041` recorded |
...
| `OAK-S10-008` | complete | `0.8.0` cut on `main` and tagged `v0.8.0`: version touch-list, `0.8.0` changelog section, `docs/release/0.8.0/release-decision.md` with its evidence table filled and its approval rows empty, regenerated OpenAPI, manual and error reference, a clean-tree image scan and a reproducible release build. The tag was cut on the owner's instruction ahead of the signatures; publication stays a separate owner decision |
