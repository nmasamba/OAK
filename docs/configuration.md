...
| `OAK_MODEL_TIMEOUT_SECONDS` | `30` | Total budget for one provider request, including the one permitted rate-limit retry. Values are clamped to 1–55 seconds: the shipped nginx proxy gives up at 60, so a longer budget would let the browser see a failure for an interpretation the API went on to commit | No |
