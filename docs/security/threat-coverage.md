...
These are the specific things the table calls "partial", plus the two residuals a **direct**
verdict still leaves: `TM-20`'s controls stop everything except a process already running as
that user, which no control on this machine can, and the verdict those controls record is a
claim about the moment it was given rather than about now. Each is in the residual-risk
register with an id.
...
| TM-20 | A stored verification verdict is a claim about the past, not proof the token is still accepted or billable | `RR-042` |
