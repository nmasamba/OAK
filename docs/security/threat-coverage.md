...
- `::test_the_reference_journey_completes_with_every_outbound_socket_broken` runs the
  brief-through-plan-compilation journey with `socket.connect`, `socket.create_connection`
  and `socket.getaddrinfo` patched to raise, and
