...
| `RR-038` | **The container images have no SBOM and no build provenance.** `docs/build/security-invariants.md` requires both for release images and bundles; the Python distribution has both, the images have neither. What they do carry is digest-pinned bases, a `make scan-images` record (`release/0.7.0/container-scan.md`), and recorded image IDs from the release workflow. Closing it means generating an SBOM per image — the scanner already used can emit one — and attaching provenance, which is only meaningful once images are published somewhere. Recorded here because that document's own preamble requires an unenforceable invariant to be written down rather than left implied. | Medium | Proposed: no | Sprint 8 |
