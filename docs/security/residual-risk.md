...
|---|---|---|---|---|
| `RR-001` | ~~Revocation notices are unsigned and the channel is fail-open.~~ **Closed 2026-08-24.** Notices are signed in the `approver` role against `revocation.schema.json` and verified against pinned anchors; a missing revocation directory, or any unreadable, oversized, malformed or unsigned notice, denies every pending dispatch (`OAK-RUNNER-REVOCATION`) instead of reading as "nothing revoked". Deleting a notice no longer restores an approval — the original attack is a regression test in `tests/integration/test_runner_revocation.py`. This was one of the two standing conditions on the `0.7.0` approval, closed before first publication while the protocol change was purely additive. | Closed | No | Sprint 5 |
