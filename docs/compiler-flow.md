...
The emitted runner plan has status `draft`, an empty approval list, and an explicit `not_signed` marker. For a read-only target profile it contains only `inventory`, `validate`, `render`, `plan`, and `verify` operations; an acknowledged non-production mutation profile additionally receives typed `apply`, `rollback`, and `destroy` operations. The compiled verification policy is derived from the same target profile and the runner enforces its clauses per requested operation kind — see [signed-runner.md](signed-runner.md). Parameters are recursively schema-checked against `command`, `shell`, `executable`, and `argv` fields. Compilation itself mutates nothing: it invokes no executable, network connection, secret resolver, runner, or target API, and the typed mutation operations it emits for an acknowledged profile are inert until separately signed, approved, and independently verified by the runner.
