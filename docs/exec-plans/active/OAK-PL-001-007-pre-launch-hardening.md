...
- [x] 2026-08-24 M8 (`OAK-PL-007`, RR-038 + RR-036 rescan): `scripts/scan_images.py`
  now emits, when `--output` is given, one CycloneDX SBOM per image
  (`oak-community-<name>-image-<version>.cdx.json`, generated by the pinned socketless
  scanner from the same exported tarball as the scan, stamped with an `oak:image`
  property binding it to its image ID) and an unsigned `image-provenance.json` shaped
  after `build_release.py`'s provenance (source commit, builder, requested platform,
  final-stage base pins distinguished from build-stage bases, image IDs, `signed:
  false`, `byte_reproducible: false`); evidence lands next to the report and stays
  out of every checksum manifest. The `images` job in `release.yml` runs the script
  (deliberate `.github` edit; the `check` job untouched), so the gate and evidence
  now also run in CI. Three new contract tests prove the provenance records the
  final stage (not the uv/node build stages), the SBOM naming convention, and the
  unsigned/unreproducible marking. **Rescan at `0.7.1`** (`docs/release/0.7.1/`):
  zero fixable findings; the rebased unprivileged web image reports **no findings at
  any severity**; the API residue is the identical unfixable 3 CRITICAL / 14 HIGH
  that `RR-036` records, plus 8 UNKNOWN entries the scanner database now carries
  without severity. `make audit` re-run clean at `0.7.1`. Governance
  `security-invariants.md` supply-chain gap rewritten as closed. `make release`
