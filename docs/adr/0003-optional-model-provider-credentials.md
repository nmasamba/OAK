...
**Discovery is explicit, with one bounded exception.** `oak models status` and `GET /v1/models` never contact a catalogue. `oak models discover` and `POST /v1/models/{family}:discover` do, and a catalogue that cannot be read produces the pinned chain labelled `pinned` rather than a pretend live answer. Since Sprint 10 an *online* interpretation also refreshes a stale catalogue and re-checks a stale token verdict before it sends anything, because "the current top model" and "this token is accepted" must be true when they are acted on, not when they were last typed. That pre-flight is bounded (15 and 5 seconds), free (anonymous reads and a generation-free request), skipped when the interpretation's own budget leaves no headroom under the proxy ceiling, and never downgrades a live snapshot to the pinned chain. A deterministic or local interpretation contacts no catalogue at all.
...
**A mode is named per request; nothing resolves to a model on its own.** `deterministic`, `online` and `local` replace `auto` and `model`. An absent choice is deterministic on every interface, which is what a client written before the model path existed sends and gets. Online AI calls the model × provider pair the user pinned, or else the preferred one: the first ungated model in the Hub's trending order, from a trusted namespace, with a live structured-output route, on the cheapest route the provider policy will offer — one the catalogue prices, and one whose published throughput says it can generate a proposal inside the request budget.

**A route is judged fast enough by arithmetic, not by trying it.** One live run produced a 3,074-token proposal on a route Hugging Face publishes at 72.5 tokens per second, and delivered about 1.57 times the published floor; those three figures (`OBSERVED_PROPOSAL_TOKENS`, `PUBLISHED_THROUGHPUT_CORRECTION`, and a `GENERATION_SHARE_OF_BUDGET` of 0.8 to leave room for the rest of the exchange) are used as observed, because the cheapest route of a model is routinely its slowest and a timed-out interpretation costs the same as a completed one. The same run is why `OAK_MODEL_TIMEOUT_SECONDS` now defaults to 35 rather than 30: a valid proposal took 29.1 seconds, and 35 is the largest default that still leaves the pre-flight its 20 seconds of headroom under the 55-second ceiling. The licence is recorded and shown rather than used to exclude, because the user chooses among pairs with it in front of them. A server-side policy suffix (`:cheapest`, `:fastest`) is not used: it could route to a provider without structured output, and only such routes are ever called.
