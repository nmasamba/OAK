...
**Interpreting a brief with a model is optional and off by default.** With nothing configured OAK contacts no provider and interprets every brief deterministically. A user may store their own key for one of seven model families and select a model, through `oak models`, the loopback `/v1/models` routes, or Settings → Models in the workspace; the key stays on that machine, in the operating-system keychain or an owner-only file, and no interface ever returns it. Once a hosted model is selected, each model-mode interpretation sends that brief to that provider under the provider's own terms ([`RR-039`](docs/security/residual-risk.md)). What the model returns is an untrusted proposal: it can write only the typed intent paths, never overwrites what the brief states, and every section it touches must be confirmed before candidates can be generated. `oak design --interpreter deterministic` never calls a provider, and the model path is permanently absent from MCP and from remote CLI mode.
