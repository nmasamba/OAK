...
**Reading a brief with a model is a per-request choice, and the default is not to.** Every brief is read deterministically unless the request says otherwise: `--interpreter online` sends it to one open model on Hugging Face Inference Providers with the user's own token, `--interpreter local` to an OpenAI-compatible server on a loopback address. The token is stored once through `oak models`, the loopback `/v1/models` routes, or Settings → Models in the workspace, in the operating-system keychain or an owner-only file; no interface ever returns it. Corroborating it is one free request to Hugging Face that generates nothing, and the verdict is recorded with the time it was given: `oak models set-key` makes that request itself unless `--no-verify` is passed, the workspace makes it straight after storing, and `oak models verify` or `POST /v1/models/{family}:verify` repeat it on demand. Online AI calls the model × provider pair the user pinned, or else the preferred one: the first model in the Hub's trending order that is ungated, comes from a trusted namespace, and has a live route that supports structured output, is priced, and is measured fast enough to answer inside `OAK_MODEL_TIMEOUT_SECONDS` — a model whose every route is too slow or unpriced is passed over and named. A pinned pair is checked against the catalogue again every time it is used, not only when it is pinned, so a route that loses structured output takes Online AI out of service with the reason rather than being called anyway. Each online interpretation sends that brief to Hugging Face and the provider it routes to, under their terms ([`RR-039`](docs/security/residual-risk.md)); the user's free monthly credit applies first and OAK promises nothing beyond it. What the model returns is an untrusted proposal: it can write only the typed intent paths, never overwrites what the brief states, and every section it touches must be confirmed before candidates can be generated. Configuring a model is permanently absent from MCP and from remote CLI mode; MCP may ask for a mode, and defaults to deterministic.
...
| [Releases](https://github.com/nmasamba/OAK/releases) | Published artifacts — wheel, sdist, SBOM, `SHA256SUMS`; checksummed, not signed, so verify with `scripts/verify_release.py` before installing |
